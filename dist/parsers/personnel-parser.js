...
export function parsePersonnelHtml(html) {
    ...
    for (let i = 0; i < t4.length; i++) {
        ...
        if (cells[0].includes('최종학교')) {
            ...
            // 라벨 없이 순서만 있는 경우
            if (!personnel.major && vRow[1])
                personnel.major = vRow[1];
            if (!personnel.degree && vRow[2])
                personnel.degree = vRow[2];
        }
    ...
    if (t9.length > 0) {
        ...
        for (let ci = 0; ci < hdr.length; ci++) {
            ...
            if ((h.includes('프로젝트') || h.includes('사업명') ||
                (h.includes('경력') && !h.includes('근거') && !h.includes('기간'))))
                cProject = ci;
            if (h.includes('주관') || h.includes('발주') ||
                (h.includes('기관') && !h.includes('교육')))
                cClient = ci;
            if (h.includes('담당 업무') || h.includes('담당분야') ||
                (h.includes('분야') && !h.includes('유사')))
                cDomain = ci;
            if (h === '역할' || (h.includes('역할') && !h.includes('분야')))
                cRole = ci;
            if (h.includes('소속') || h.includes('수행사') ||
                (h.includes('회사') && !h.includes('기관')))
                cCompany = ci;
            if (h.includes('비고') || h.includes('근거'))
                cRemarks = ci;
        }
