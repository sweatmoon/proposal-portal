 ...
 *
 * NAS "99.악티보포털참조용/02.제안/03.자격증사본" 폴더(하위 "자격증(상근)" / "자격증(비상근)")에서
 * "자격증(이름).pptx" 파일을 찾아 → extractAllImagesFromPptx()로 슬라이드별 이미지 추출 →
 * 범용 템플릿(도장O/도장X 선택 가능)의 큰 이미지 자리에 슬라이드로 삽입한다.
 * 자격증이 여러 슬라이드인 경우 모든 페이지를 순서대로 삽입한다.
 *
 * 동작 흐름:
 *   1. proposal_members 에서 이 사업 투입 인력 이름 목록 조회
 *   2. fetchAuditorCertificatePptxs(names) — NAS 서브폴더 탐색, 이름별 PPTX 취득
 *   3. 이름 순서대로 각 PPTX → extractAllImagesFromPptx() → PNG 배열 (다중 슬라이드 지원)
 *   4. buildStampedDeckZip() 으로 슬라이드 조립
 *      - withStamp=true  → fetchCompanyStampPng() 결과를 작은 자리에 삽입
 *      - withStamp=false → 작은 자리 XML 요소 제거
 *   5. PPTX를 찾지 못한 인력은 skipped 배열에 수집
 *
...
export async function buildLicenseCertificateZip(
  ...
  if (personCount === 0) {
    ...
    throw new Error(
      `NAS 자격증사본 폴더에서 투입 인력 중 매칭되는 PPTX가 한 개도 없습니다 (${nameList})`
    )
  }
  ...
  // 인원별 이미지 배열 수집 — PPTX의 모든 슬라이드 이미지를 순서대로 펼침
  ...
  for (const name of names) {
    ...
    const pages = await extractAllImagesFromPptx(pptxBuf)
    if (!pages.length) {
      // 이미지 추출 실패 시 건너뜀
      skipped.push(name)
      continue
  ...
  if (!allBigImages.length) {
    throw new Error('자격증사본 PPTX에서 이미지를 추출하지 못했습니다')
  }
